      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
            <div>
                 ...
                 <p className="text-sm text-muted-foreground">Xem và quản lý bình luận trên các truyện của bạn</p>
            </div>
                ...
                {loading ? (
                    <div className="p-6 space-y-4">
                     ...
                     <div className="text-center py-12 flex flex-col items-center gap-3">
                        ...
                        <p className="text-muted-foreground">Chưa có bình luận nào trên các bộ truyện của bạn</p>
                    </div>
                ) : (
                    <Table>
                        <TableHeader>
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            <Avatar className="h-8 w-8">
                                                <AvatarImage src={typeof comment.user !== 'string' ? comment.user?.avatarUrl : undefined} />
                                                <AvatarFallback>
                                                    {typeof comment.user !== 'string' ? comment.user?.username?.charAt(0).toUpperCase() : '?'}
                                                </AvatarFallback>
                                            </Avatar>
                                            <div className="flex flex-col">
                                                <span className="font-medium text-sm">
                                                    {typeof comment.user !== 'string' ? comment.user?.username : 'Unknown'}
                                                </span>
                                                <span className="text-xs text-muted-foreground">{typeof comment.user !== 'string' ? comment.user?.role : ''}</span>
                                            </div>
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell>
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-1 font-medium text-sm">
                                                <span>{typeof comment.manga !== 'string' ? comment.manga?.title : ''}</span>
                                            </div>
                                            {comment.chapter && typeof comment.chapter !== 'string' && (
